...
export default class ProjectExportsList extends React.Component<ProjectExportsListProps, ProjectExportsListState> {
  ...
  addExportFetcher(exportUid: string) {
    ...
    this.exportFetchers.set(exportUid, new ExportFetcher(this.props.asset.uid, exportUid))
  }
  ...
  render() {
    ...
    } else if (this.state.rows.length === 0 || this.state.selectedExportType.isLegacy) {
      return null
    } else {
      return (
        <React.Fragment>
